/**
 * NEXUS ESPORTS — create-policy.js
 */
...
const PolicyPage = (() => {
  ...
  const initCreate = () => {
    ...
    const clauseEditor = buildClauseEditor('clause-list');
    const tagInput     = buildTagInput('tag-input-wrap');
    window.__addClause = () => clauseEditor.addClause();
    setupDateValidation();

    document.getElementById('btn-draft')?.addEventListener('click', () => {
      const data = collectForm(clauseEditor, tagInput);
      if (!data.title) { toast('Enter a title before saving draft.', 'error'); return; }
      const newId = 'pol-' + Date.now();
      policies.push({ id: newId, version: 'v1.0', updatedBy: 'Admin', updatedAt: new Date().toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}), changelog: [{ ver:'v1.0', date: new Date().toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}), desc: 'Initial draft created.' }], ...data, status: 'draft' });
      save();
